...
export default function StockPallet({ user }) {
  ...
  // --- Excel Import Handlers for Matrix and Standard Formats ---
  const [workbook, setWorkbook] = useState(null);
  const [sheetNames, setSheetNames] = useState([]);
  const [selectedSheetName, setSelectedSheetName] = useState('');

  // Matrix parser (Sidoarjo Manual Excel Sheet)
  const parseWorksheet = (worksheet) => {
    const rows = XLSX.utils.sheet_to_json(worksheet, { header: 1 });
    
    if (rows.length === 0) {
      throw new Error('Excel kosong atau format tidak sesuai!');
    }

    const getCellComment = (r, c) => {
      if (!XLSX.utils || !XLSX.utils.encode_cell) return '';
      const cellAddress = XLSX.utils.encode_cell({ r, c });
      const cell = worksheet[cellAddress];
      if (cell && cell.c && cell.c.length > 0) {
        return cell.c.map(comm => comm.t).join(' ').replace(/USER:/gi, '').replace(/\n/g, ' - ').trim();
      }
      return '';
    };

    let isMatrixFormat = false;
    let periodeDate = new Date().toISOString().split('T')[0];
    let idxCust = 1, idxUkuran = 2, idxSubcont = 3, idxCode = 4;

    for (let r = 0; r < Math.min(rows.length, 25); r++) {
      const row = rows[r];
      if (!row) continue;
      
      // Detect column offset based on "CUSTOMER" header
      for (let c = 0; c < Math.min(row.length, 10); c++) {
        if (String(row[c] || '').toUpperCase().trim() === 'CUSTOMER') {
          idxCust = c;
          idxUkuran = c + 1;
          idxSubcont = c + 2;
          idxCode = c + 3;
      ...
      for (let c = 0; c < row.length; c++) {
        ...
        if (cellVal.includes('periode')) {
          const months = {
            januari: '01', februari: '02', maret: '03', april: '04', mei: '05', juni: '06',
            juli: '07', agustus: '08', september: '09', oktober: '10', november: '11', desember: '12',
            january: '01', february: '02', march: '03', may: '05', june: '06', july: '07', august: '08',
            october: '10', december: '12'
          };
          
          for (const innerCell of row) {
            const innerStr = String(innerCell || '').toLowerCase();
            for (const mName of Object.keys(months)) {
              if (innerStr.includes(mName)) {
                const yearMatch = innerStr.match(/\d{4}/);
                const year = yearMatch ? yearMatch[0] : new Date().getFullYear();
                periodeDate = `${year}-${months[mName]}-01`;
                break;
              }
            }
          }
        ...
        if (cellVal === '1' && String(row[c + 1] || '') === '2' && String(row[c + 2] || '') === '3') {
          isMatrixFormat = true;
        }
      }
    }
    ...
    if (isMatrixFormat) {
      // ── Step 1: find date header row (row with 1,2,3,4... up to ~31) ──
      let dateColStart = -1;  // column index where day 1 sits
      let dateRowIdx   = -1;
      ...
      for (let r = 0; r < Math.min(rows.length, 30); r++) {
        ...
        if (dateColStart !== -1) break;
      }

      // ── Step 2: build column→date map ────────────────────────────
      const colToDate = {}; // col index → 'YYYY-MM-DD'
      if (dateColStart !== -1 && dateRowIdx !== -1) {
        const dateHeaderRow = rows[dateRowIdx];
        const [baseYear, baseMon] = periodeDate.split('-').map(Number);
        for (let c = dateColStart; c < dateHeaderRow.length; c++) {
          const dayNum = Number(dateHeaderRow[c]);
          if (!isNaN(dayNum) && dayNum >= 1 && dayNum <= 31) {
            // Validate day exists for this month
            const d = new Date(baseYear, baseMon - 1, dayNum);
            if (d.getMonth() === baseMon - 1) {
              const yearStr = String(baseYear);
              const monthStr = String(baseMon).padStart(2, '0');
              const dayStr = String(dayNum).padStart(2, '0');
              colToDate[c] = `${yearStr}-${monthStr}-${dayStr}`;
            }
          }
        }
      }

      const hasDateCols = Object.keys(colToDate).length > 0;

      // ── Step 3: parse customer blocks ────────────────────────────
      // For each customer block, if we have day columns we emit one
      // row per day that has non-zero values. Otherwise fall back to
      // a single aggregate row (old behaviour).

      let customerBlocks = []; // { customer, ukuran, stockAwal, dayData: { colIdx: {M,K,R...} } }
      ...
      for (let r = 0; r < rows.length; r++) {
        ...
        const colB = String(row[idxCust] || '').trim();
        const colC = String(row[idxUkuran] || '').trim();
        const colD = String(row[idxSubcont] || '').trim().toUpperCase();
        const colE = String(row[idxCode] || '').trim().toUpperCase();

        if (colB !== '') lastCustomer = colB;
        if (colC !== '') lastUkuran = colC;

        if (colE === 'A' && lastCustomer !== '') {
          if (currentBlock) customerBlocks.push(currentBlock);

          let stockAwal = 0;
          if (hasDateCols) {
            // stockAwal is the value in the FIRST date column for the 'A' row
            const firstDateCol = Math.min(...Object.keys(colToDate).map(Number));
            if (row[firstDateCol] !== undefined && row[firstDateCol] !== '' && !isNaN(Number(row[firstDateCol]))) {
              stockAwal = Number(row[firstDateCol]);
            }
          } else {
            for (let c = 6; c <= 36; c++) {
              if (row[c] !== undefined && row[c] !== '' && !isNaN(Number(row[c]))) {
                stockAwal = Number(row[c]); break;
              }
            }
          }
          ...
          currentBlock = {
            ...
            ukuran: lastUkuran || '1000x1200 mm',
            stockAwal,
            rows: [] // { colE, colD, rowData, rowIndex }
          };
        } else if (currentBlock && colE) {
          currentBlock.rows.push({ colE, colD, rowData: row, rowIndex: r });
        }
      }
      if (currentBlock) customerBlocks.push(currentBlock);

      // ── Step 4: emit transaction rows ────────────────────────────
      for (const block of customerBlocks) {
        if (hasDateCols) {
          // Group by date column
          const dateCols = Object.keys(colToDate).map(Number).sort((a, b) => a - b);

          let runningStock = block.stockAwal;

          for (const col of dateCols) {
            let produksi = 0, dariLumajang = 0, dariSubcont = 0;
            let palletKeluar = 0, returLumajang = 0, returCustomer = 0;
            let subcontNames = new Set();
            let hasAnyValue = false;

            for (const { colE, colD, rowData, rowIndex } of block.rows) {
              ...
              const comment = getCellComment(rowIndex, col);
              if (comment && colE !== 'P' && colE !== 'PRODUKSI' && colE !== 'RAKIT WH') {
                subcontNames.add(comment);
              }

              hasAnyValue = true;
              if (colE === 'P' || colE === 'PRODUKSI' || colE === 'RAKIT WH') { produksi += val; }
              else if (colE === 'M' || colE === 'MS') {
                if (colD.includes('WS') || colD.includes('LUMAJANG') || colD.includes('LMJ')) {
                  dariLumajang += val;
                } else {
                  dariSubcont += val;
                  if (colD && !['SUBCONT', 'M', 'MS'].includes(colD)) subcontNames.add(colD);
                }
              } else if (colE === 'K') { palletKeluar += val; }
              else if (colE.includes('R. CUST') || colE.includes('RETUR CUST') || colE === 'RC') { returCustomer += val; }
              else if (colE.includes('R. WS') || colE.includes('RETUR WS') || colE.includes('WS') || colE.includes('REJECT') || colE === 'RW' || colE === 'REJECT KE WS') { returLumajang += val; }
            }

            const totalTx = produksi + dariLumajang + dariSubcont + palletKeluar + returLumajang + returCustomer;
            if (totalTx === 0) {
              // Update running stock even if we skip pushing
              // Note: if all tx are 0, runningStock doesn't change, but just in case
              continue;
            }

            mapped.push({
              id: 'sp_imp_' + Math.random().toString(36).substring(2, 9) + '_' + Date.now(),
              tanggal: colToDate[col],
              customer: block.customer,
              ukuran: block.ukuran,
              produksi,
              stockAwal: runningStock,
              dariLumajang,
              dariSubcont,
              subcontNama: Array.from(subcontNames).join(', '),
              palletKeluar,
              returLumajang,
              returCustomer
            });

            // Update running stock for the next transaction day
            runningStock = runningStock + produksi + dariLumajang + dariSubcont + returCustomer - palletKeluar - returLumajang;
          }
        } else {
          // Fallback: single aggregate row (old behaviour)
          let produksi = 0, dariLumajang = 0, dariSubcont = 0;
          let palletKeluar = 0, returLumajang = 0, returCustomer = 0;
          let subcontNames = new Set();
          ...
          for (const { colE, colD, rowData, rowIndex } of block.rows) {
            let totalVal = 0;
            let valColIdx = -1;
            if (rowData[40] !== undefined && rowData[40] !== '' && !isNaN(Number(rowData[40]))) {
              totalVal = Number(rowData[40]);
              valColIdx = 40;
            } else {
              for (let c = Math.min(rowData.length - 1, 45); c >= 6; c--) {
                if (rowData[c] !== undefined && rowData[c] !== '' && !isNaN(Number(rowData[c]))) {
                  totalVal = Number(rowData[c]); 
                  valColIdx = c;
                  break;
                }
              }
            ...
            if (totalVal > 0 && valColIdx !== -1) {
              const comment = getCellComment(rowIndex, valColIdx);
              if (comment && colE !== 'P' && colE !== 'PRODUKSI' && colE !== 'RAKIT WH') {
                subcontNames.add(comment);
              }
            }

            if (colE === 'P' || colE === 'PRODUKSI' || colE === 'RAKIT WH') { produksi += totalVal; }
            else if (colE === 'M' || colE === 'MS') {
              if (colD.includes('WS') || colD.includes('LUMAJANG') || colD.includes('LMJ')) {
                dariLumajang += totalVal;
              } else {
                dariSubcont += totalVal;
                if (colD && !['SUBCONT', 'M', 'MS'].includes(colD)) subcontNames.add(colD);
              }
            } else if (colE === 'K') { palletKeluar += totalVal; }
            else if (colE.includes('R. CUST') || colE.includes('RETUR CUST') || colE === 'RC') { returCustomer += totalVal; }
            else if (colE.includes('R. WS') || colE.includes('RETUR WS') || colE.includes('WS') || colE.includes('REJECT') || colE === 'RW' || colE === 'REJECT KE WS') { returLumajang += totalVal; }
          }
      ...
      for (let r = 1; r < rows.length; r++) {
        ...
        const getVal = (possibleHeaders, defaultValue = 0) => {
          for (let c = 0; c < row.length; c++) {
            const header = headers[c] || '';
            if (possibleHeaders.some(h => header.includes(h))) {
              return isNaN(Number(row[c])) ? defaultValue : Number(row[c]);
            }
          }
          return defaultValue;
        };

        const getStringVal = (possibleHeaders, defaultValue = '') => {
          for (let c = 0; c < row.length; c++) {
            const header = headers[c] || '';
            if (possibleHeaders.some(h => header.includes(h))) {
              return row[c] !== undefined && row[c] !== null ? String(row[c]).trim() : defaultValue;
            }
          }
          return defaultValue;
        };

        const customer = getStringVal(['customer', 'nama', 'perusahaan']);
        if (!customer) continue;

        let dateVal = getStringVal(['tgl', 'tanggal', 'date'], null);
        if (dateVal !== null && dateVal !== undefined && dateVal !== '') {
          try {
            const numDate = Number(dateVal);
            if (!isNaN(numDate) && numDate > 40000 && numDate < 60000) {
              const parsedDate = new Date((numDate - 25569) * 86400 * 1000);
              if (!isNaN(parsedDate.getTime())) {
                dateVal = parsedDate.toISOString().split('T')[0];
              }
            } else {
              const trimmedDate = String(dateVal).trim();
            ...
            console.error("Gagal mengurai tanggal mutasi:", dateVal, e);
          }
        }
        ...
        const produksi = getVal(['produksi', 'prod'], 0);
        const dariLumajang = getVal(['lumajang', 'dari lumajang', 'lmj'], 0);
        const dariSubcont = getVal(['subcont', 'dari subcont', 'sub'], 0);
        const palletKeluar = getVal(['keluar', 'pallet keluar', 'kirim'], 0);
        const returLumajang = getVal(['retur lumajang', 'retur lmj', 'ws'], 0);
        const returCustomer = getVal(['retur customer', 'retur cust', 'retur c'], 0);
        ...
        mapped.push({
          ...
          tanggal: dateVal,
          customer: customer,
          ukuran: getStringVal(['ukuran', 'size'], '1000x1200 mm'),
          produksi,
          stockAwal: getVal(['awal', 'stok awal', 'stock awal'], 0),
          dariLumajang,
          dariSubcont,
          palletKeluar,
          returLumajang,
    ...
    if (mapped.length === 0) {
      throw new Error('Tidak ada data yang dapat dibaca. Pastikan format spreadsheet sesuai.');
    }
