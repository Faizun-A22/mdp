...
const parseWorksheet = (worksheet, sheetName) => {
  ...
  headers.forEach((h, i) => {
    ...
    if (lh.includes('tgl') || lh.includes('tanggal') || lh.includes('date')) {
      colMap.tanggal = i;
...
details.forEach((d) => {
  if (d.totalItems > 0) {
    if (d.outstandingItems > 0) {
      console.log(`- ${d.sheetName}: Parsed ${d.totalItems} POs (${d.outstandingItems} outstanding)`);
    }
  } else if (d.excelRowsCount > 0) {
    console.log(`- ${d.sheetName}: FAILED parsing (Excel rows count: ${d.excelRowsCount})`);
    console.log('  Headers / Sample rows:', JSON.stringify(d.sampleFirstRow));
  }
});
