  ...
  dataRows.forEach((row, idx) => {
    ...
    if (poRaw) {
      ...
      if (tenggatRaw) {
        ...
        if (tenggatStr && !isNaN(new Date(tenggatStr).getTime())) {
          activeTenggat = tenggatStr;
        }
      }

      if (custRaw) activeCustomer = String(custRaw).trim();
      else activeCustomer = String(sheetName || 'PT Unknown').trim();
...
const wb = XLSX.readFile('c:\\mdp\\test_sheet_sync.xlsx');
let totalSheets = wb.SheetNames.length;
...
wb.SheetNames.forEach(sheetName => {
  ...
  if (items.length > 0) {
    ...
    // Count outstanding (sisaPo > 0)
    const outstandingCount = items.filter(x => x.sisaPo > 0).length;
    
    details.push({
      sheetName,
      totalItems: items.length,
      outstandingItems: outstandingCount
    });
  } else {
    // Let's see what is inside this sheet to understand why it has 0 items
    ...
    details.push({
      ...
      sampleFirstRow: rows.slice(0, 4)
    });
  }
});

console.log(`Total Sheets: ${totalSheets}`);
console.log(`Sheets with data: ${sheetsWithData}`);
console.log(`Total Rows Parsed: ${totalRowsParsed}`);

console.log('\n--- Details of Checked Sheets (INDOFOOD, GARUDA FOOD, KENCANA ABADI, ALDZAMA, ARTHAWENA) ---');
const targetSheets = ['INDOFOOD', 'GARUDA FOOD', 'KENCANA ABADI', 'ALDZAMA', 'ARTHAWENA'];
details.forEach((d) => {
  if (targetSheets.includes(d.sheetName)) {
    console.log(`- ${d.sheetName}: Parsed ${d.totalItems} POs (${d.outstandingItems} outstanding)`);
  }
});
